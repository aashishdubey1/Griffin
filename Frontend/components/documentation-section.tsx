...
export default function DocumentationSection() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Badge
            icon={
              <div
                className={`w-[10.50px] h-[10.50px] outline outline-[1.17px] ${isDarkMode ? "outline-[#e5e5e5]" : "outline-[#37322F]"} outline-offset-[-0.58px] rounded-full transition-colors duration-200`}
              ></div>
            }
            text="Platform Features"
          />
          <div
            className={`self-stretch text-center flex justify-center flex-col ${isDarkMode ? "text-[#e5e5e5]" : "text-[#49423D]"} text-xl sm:text-2xl md:text-3xl lg:text-5xl font-semibold leading-tight md:leading-[60px] font-sans tracking-tight transition-colors duration-500`}
          >
      ...
      {/* Content Section */}
      <div
        className={`self-stretch px-4 md:px-9 overflow-hidden flex justify-start items-center ${isDarkMode ? "bg-[#1a1a1a]" : "bg-[#F7F5F3]"} transition-colors duration-500`}
      >
        <div className="flex-1 py-8 md:py-11 flex flex-col md:flex-row justify-start items-center gap-6 md:gap-12">
                ...
                <div
                  ...
                  className={`w-full overflow-hidden flex flex-col justify-start items-start transition-all duration-300 cursor-pointer rounded-lg ${
                    isActive
                      ...
                      : `${isDarkMode ? "border border-[rgba(255,255,255,0.15)]" : "border border-[rgba(2,6,23,0.08)]"} ${isDarkMode ? "hover:bg-[rgba(255,255,255,0.08)]" : "hover:bg-white/50"}`
                  } hover:scale-[1.02]`}
                >
                  <div
                    className={`w-full h-0.5 ${isDarkMode ? "bg-[rgba(255,255,255,0.08)]" : "bg-[rgba(50,45,43,0.08)]"} overflow-hidden ${isActive ? "opacity-100" : "opacity-0"} transition-opacity duration-300`}
                  >
                    <div
                      key={animationKey}
                      className={`h-0.5 ${isDarkMode ? "bg-gradient-to-r from-[#e5e5e5] to-[#f5f5f5]" : "bg-gradient-to-r from-[#322D2B] to-[#4a4540]"} animate-[progressBar_5s_linear_forwards] will-change-transform`}
                    />
                  </div>
                  <div className="px-6 py-5 w-full flex flex-col gap-2">
                    <div
                      className={`self-stretch flex justify-center flex-col ${isDarkMode ? "text-[#e5e5e5]" : "text-[#49423D]"} text-sm font-semibold leading-6 font-sans transition-colors duration-500`}
                    >
                      {card.title}
                    </div>
                    <div
                      className={`self-stretch ${isDarkMode ? "text-[rgba(229,229,229,0.70)]" : "text-[#605A57]"} text-[13px] font-normal leading-[22px] font-sans whitespace-pre-line transition-colors duration-500`}
                    >
                      {card.description}
                    </div>
                  </div>
          ...
          <div className="w-full md:w-auto rounded-lg flex flex-col justify-center items-center gap-2 order-1 md:order-2 md:px-0 px-0">
            <div
              className={`w-full md:w-[580px] h-[250px] md:h-[420px] ${isDarkMode ? "bg-[#2a2a2a]" : "bg-white"} ${isDarkMode ? "shadow-[0px_0px_0px_0.9056603908538818px_rgba(255,255,255,0.15)]" : "shadow-[0px_0px_0px_0.9056603908538818px_rgba(0,0,0,0.08)]"} overflow-hidden rounded-lg flex flex-col justify-start items-start transition-all duration-500 hover:scale-[1.02] group`}
            >
